...
export function extractExperienceYears(cvText: string): number {
  if (!cvText) return 0;
...
export function extractOccupationalRoles(cvText: string): string[] {
  if (!cvText) return [];
  ...
  const foundRoles: Set<string> = new Set();
...
export function extractEducation(cvText: string): string[] {
  if (!cvText) return [];
  ...
  const foundEducation: Set<string> = new Set();
