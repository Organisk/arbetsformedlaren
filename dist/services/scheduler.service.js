 ...
 */
export async function runSchedulerCycle(config) {
    const result = {
        success: false,
        jobsFetched: 0,
        jobsNew: 0,
    ...
    try {
        ...
        const existingJobs = db.prepare('SELECT external_id FROM jobs WHERE removed = 0').all();
        ...
        for (const job of searchResults) {
          ...
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `);
                stmt.run(job.id, job.external_id, job.headline, job.company, job.municipality, job.county, job.employment_type, job.duration, job.scope_of_work, job.lat, job.lon, JSON.stringify(job.must_skills), JSON.stringify(job.must_languages), job.occupation, job.occupation_group, job.publication_url, job.employer_name, job.salary_type, job.publication_date);
                newJobs.push(job);
                result.jobsNew++;
            }
        }
        ...
        if (newJobs.length > 0 && config.userIds.length > 0) {
            ...
            const users = db.prepare(`SELECT * FROM users WHERE id IN (${userPlaceholders})`).all();
                ...
                for (const job of newJobs) {
                    ...
                    if (cvData.skills && cvData.skills.length > 0) {
                        ...
                        if (matchResult.totalScore >= 30) { // Only show matches with >= 30% score
                            ...
                            const existingMatch = db.prepare('SELECT * FROM matches WHERE user_id = ? AND job_id = ? AND status = ?').get(user.id, job.id, 'new');
                            if (!existingMatch) {
                                const matchStmt = db.prepare(`
                  INSERT INTO matches (id, user_id, job_id, score, match_reasons, status)
                  VALUES (?, ?, ?, ?, ?, ?)
                `);
                                matchStmt.run(`match_${user.id}_${job.id}`, user.id, job.id, matchResult.totalScore, JSON.stringify(matchResult.reasons), 'new');
 ...
 */
export function getSchedulerStatus(config) {
    let mode = 'interval';
