 ...
 */
function calculateSkillsScore(cvSkills, jobRequiredSkills) {
 ...
 */
function calculateExperienceScore(cvYears, jobRequiredYears) {
 ...
 */
function calculateOccupationScore(cvOccupation, jobOccupation) {
 ...
 */
function calculateEducationScore(cvEducation, jobRequiredEducation) {
 ...
 */
function calculateLanguagesScore(cvLanguages, jobRequiredLanguages) {
 ...
 */
function calculateLocationScore(userLat, userLon, jobLat, jobLon, maxRadiusKm) {
...
export function matchCVagainstJob(cv, job, userLocation, maxRadiusKm = 40) {
    ...
    const { score: skillsScore, matched: matchedSkills, missing: skillsMissing } = calculateSkillsScore(cv.skills, job.must_skills);
    // Experience: from CV work experience years vs job scope_of_work
    const cvYears = parseInt(cv.work_experiences[0] || '0', 10);
    const { score: experienceScore, message: experienceMessage } = calculateExperienceScore(cvYears, job.scope_of_work);
    // Occupation: cv occupational roles vs job occupation
    const { score: occupationScore, message: occupationMessage, related } = calculateOccupationScore(cv.occupational_roles.join(', '), job.occupation);
    // Education: cv education vs job must_education
    const { score: educationScore, matched: matchedEducation, missing: educationMissing } = calculateEducationScore(cv.education, job.must_education || []);
    // Languages: cv languages vs job must_languages
    const { score: languagesScore, matched: matchedLanguages, missing: languagesMissing } = calculateLanguagesScore(cv.languages, job.must_languages || []);
    // Location: geographic distance
    const { score: locationScore, distanceKm, withinRadius } = calculateLocationScore(userLocation?.lat ?? null, userLocation?.lon ?? null, job.lat, job.lon, maxRadiusKm);
