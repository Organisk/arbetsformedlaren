 ...
 */
export function analyzeCV(cvText, config = {}) {
 ...
 */
function analyzeCVRuleBased(cvText) {
    ...
    if (missingCompetencies.length === 0)
        score += 20;
    if (/(?:år|erfarenhet)/.test(text))
        score += 15;
    if (/(?:utbildning|examen)/.test(text))
        score += 15;
    if (/(?:engelska|språk)/.test(text))
        score += 10;
 ...
 */
export function compareCVWithJob(cvText, jobTitle, jobDescription, config = {}) {
    const { openaiApiKey } = config;
 ...
 */
function compareCVWithJobRuleBased(cvText, jobTitle, jobDescription) {
    const matchReasons = [];
    const missingForMatch = [];
    const adjustments = [];
 ...
 */
export function generateCoverLetter(cvText, jobTitle, company, config = {}) {
    const { openaiApiKey } = config;
 ...
 */
function generateCoverLetterRuleBased(cvText, jobTitle, company) {
    const text = cvText.toLowerCase();
